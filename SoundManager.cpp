/*
fmod Sound Manager for OGRE
http://www.ogre3d.org/wiki/index.php/FMOD_SoundManager
...
void SoundManager::Initialize(void)
   ...
   initResult = FMOD::System_Create(&system);
   /*
   if (result != FMOD_OK)
   ...
   initResult = system->init(MAX_SOUND_CHANNELS, FMOD_INIT_NORMAL, 0);	// Initialize FMOD.
   ...
   initResult = system->setFileSystem(&fmodFileOpenCallback, &fmodFileCloseCallback, &fmodFileReadCallback, &fmodFileSeekCallback, 2048);
   /*
   if (result != FMOD_OK)
      OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR, "FMOD error! (" + StringConverter::toString(result) + "): " + FMOD_ErrorString(result), "SoundManager::Initialize");
  */
